...
void mt_environment_init(
    ...
    env->skybox_pipeline = (MtPipelineAsset *)mt_asset_manager_load(
        asset_manager, "../assets/shaders/skybox.glsl");
...
void mt_environment_draw_skybox(MtEnvironment *env, MtCmdBuffer *cb) {
    ...
    if (env->skybox_asset) {
        mt_render.cmd_bind_image(
            cb, env->skybox_asset->image, env->regular_sampler, 1, 1);
    } else {
        mt_render.cmd_bind_image(
            cb, engine->black_image, env->regular_sampler, 1, 1);
    }
