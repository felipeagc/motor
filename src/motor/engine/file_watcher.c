...
static void
watcher_recursive_add(MtFileWatcher *w, char *path_buffer, size_t path_len, size_t path_max)
{
    ...
    while ((ent = readdir(dirp)) != 0x0)
    {
        ...
        watcher_recursive_add(w, path_buffer, path_len + d_name_size + 1, path_max);
    }
    ...
    MtAllocator *alloc, MtFileWatcherEventType types, MtFileWatcherHandler handler, const char *dir)
{
    ...
    if (types & MT_FILE_WATCHER_EVENT_REMOVE)
        w->watch_flags |= IN_DELETE;
    if (types & MT_FILE_WATCHER_EVENT_MODIFY)
        w->watch_flags |= IN_MODIFY;
    if (types & MT_FILE_WATCHER_EVENT_MOVE)
...
static const char *find_wd_path(MtFileWatcher *w, int wd)
{
    for (size_t i = 0; i < mt_array_size(w->items); ++i)
        if (wd == w->items[i].wd)
            return w->items[i].path;
    return 0x0;
}

static char *build_full_path(MtFileWatcher *watcher, int wd, const char *name, uint32_t name_len)
{
...
void mt_file_watcher_poll(MtFileWatcher *w, void *user_data)
{
    ...
    if (read_bytes > 0)
    {
        for (char *bufp = read_buffer; bufp < read_buffer + read_bytes;)
        {
            ...
            {
                MtFileWatcherEvent e = {MT_FILE_WATCHER_EVENT_BUFFER_OVERFLOW, NULL, NULL};
                handler(&e, user_data);
            }
            else
            {
                ...
                else if (is_move_to)
                {
                    if (move_src && move_cookie == ev->cookie)
                    {
                        ...
                        char *dst            = build_full_path(w, ev->wd, ev->name, ev->len);
                    ...
                    else if (move_src != 0x0)
                    {
                        ...
                        char *dst = build_full_path(w, ev->wd, ev->name, ev->len);
                        e         = (MtFileWatcherEvent){MT_FILE_WATCHER_EVENT_MOVE, NULL, dst};
                        handler(&e, user_data);
                        mt_free(w->alloc, dst);
                    }
                    else
                    {
                        ...
                        char *dst            = build_full_path(w, ev->wd, ev->name, ev->len);
                        MtFileWatcherEvent e = {MT_FILE_WATCHER_EVENT_MOVE, NULL, dst};
                        handler(&e, user_data);
                        mt_free(w->alloc, dst);
                    }
                }
...
static char *build_full_path(MtFileWatcher *w, FILE_NOTIFY_INFORMATION *ev)
{
    ...
    for (size_t i = 0; i < name_len; ++i)
    {
        ...
        res[w->watch_dir_len + 1 + i] = ev->FileName[i] > 255 ? '_' : (char)ev->FileName[i];
