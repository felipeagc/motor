...
static void *bump_realloc(BumpAllocator *alloc, void *ptr, uint64_t size)
{
    if (ptr == NULL && size > 0)
    {
        ...
        {
            // Append new block
            alloc->last_block->next = mt_alloc(alloc->parent_allocator, sizeof(BumpBlock));
...
void mt_bump_alloc_init(MtAllocator *alloc, uint64_t block_size, MtAllocator *parent_allocator)
{
